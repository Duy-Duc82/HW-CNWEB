...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (addProductBtn && addProductForm) {
    // Make the form collapsible via CSS transitions
    addProductForm.classList.add('collapsible');

    addProductBtn.addEventListener('click', function (e) {
      e.preventDefault();
      // Toggle visual open class
      addProductForm.classList.toggle('is-open');
      // Also toggle 'hidden' for accessibility/fallback (keeps original behavior)
      if (addProductForm.classList.contains('is-open')) {
        addProductForm.classList.remove('hidden');
      } else {
        // Delay adding hidden until transition ends for smoother UX
        setTimeout(function () { addProductForm.classList.add('hidden'); }, 500);
      }
      // Nếu mới hiện form, focus vào input tên (delay to allow transition)
      if (addProductForm.classList.contains('is-open')) {
        setTimeout(function () {
